...
#define FOR(i, n) for (i = 0; i < (n); ++i)
    ...
    {
        int i;
        FOR(i, 16) o[i] = n[2 * i] + (static_cast<i64>(n[2 * i + 1]) << 8);
        o[15] &= 0x7fff;
    }
    ...
    static void modL(u8* r, i64 x[64])
    {
        ...
        for (i = 63; i >= 32; --i)
        {
            ...
            for (j = i - 32; j < i - 12; ++j)
            {
                x[j] += carry - 16 * x[i] * static_cast<i64>(L[j - (i - 32)]);
                carry = (x[j] + 128) >> 8;
                x[j] -= carry << 8;
            }
            x[j] += carry;
        ...
        FOR(j, 32)
        {
            x[j] += carry - (x[31] >> 4) * static_cast<i64>(L[j]);
            carry = x[j] >> 8;
            x[j] &= 255;
        }
        FOR(j, 32) x[j] -= carry * static_cast<i64>(L[j]);
        FOR(i, 32)
        {
            x[i + 1] += x[i] >> 8;
            r[i] = x[i] & 255;
    ...
    {
        i64 x[64], i;
        FOR(i, 64) x[i] = static_cast<u64>(r[i]);
        FOR(i, 64) r[i] = 0;
        modL(r, x);
    }

    static void seed_keypair(u8* pk, u8* sk, const u8* seed)
    {
        u8 d[64];
        gf p[4];
        int i;
    ...
    static void sign_detached(u8* sig, const u8* m, u64 n, const u8* sk)
    {
        std::vector<u8> sm(n + 64);
        u8 d[64], h[64], r[64];
        i64 i, j, x[64];
        gf p[4];
        ...
        crypto_hash(r, sm.data() + 32, n + 32);
        reduce(r);
        scalarbase(p, r);
        pack(sm.data(), p);

        FOR(i, 32) sm[i + 32] = sk[i + 32];
        crypto_hash(h, sm.data(), n + 64);
        reduce(h);

        FOR(i, 64) x[i] = 0;
        FOR(i, 32) x[i] = static_cast<u64>(r[i]);
        FOR(i, 32) FOR(j, 32) x[i + j] += h[i] * static_cast<u64>(d[j]);
        modL(sm.data() + 32, x);
        std::memcpy(sig, sm.data(), 64);
    }

    static const char kB64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

    static std::string b64(const u8* data, size_t len)
    {
        std::string out;
...
std::vector<unsigned char> ed25519_seed_from_pkcs8_der(const std::vector<unsigned char>& der)
{
    ...
    static const unsigned char kOid[] = {0x06, 0x03, 0x2b, 0x65, 0x70};
    const auto it = std::search(der.begin(), der.end(), std::begin(kOid), std::end(kOid));
    if (it == der.end())
        throw std::runtime_error("PKCS#8 is not an Ed25519 private key (OID 1.3.101.112 not found)");

    size_t i = static_cast<size_t>(it - der.begin()) + sizeof(kOid);
    while (i < der.size() && der[i] != 0x04)
        ++i;
    if (i + 1 >= der.size())
        throw std::runtime_error("PKCS#8 Ed25519 private key OCTET STRING missing");

    size_t len = der[i + 1];
    i += 2;
    if (len == 0x22 && i + 2 <= der.size() && der[i] == 0x04 && der[i + 1] == 0x20)
    {
        i += 2;
        len = 32;
    }
    if (len < 32 || i + 32 > der.size())
        throw std::runtime_error("PKCS#8 Ed25519 seed is not 32 bytes");
    return {der.begin() + static_cast<std::ptrdiff_t>(i),
            der.begin() + static_cast<std::ptrdiff_t>(i + 32)};
}

std::string ed25519_sign_b64(const std::vector<unsigned char>& seed, const std::string& message)
{
    if (seed.size() != 32)
        throw std::runtime_error("Ed25519 seed must be 32 bytes");

    u8 pk[32], sk[64], sig[64];
    seed_keypair(pk, sk, seed.data());
    sign_detached(sig, reinterpret_cast<const u8*>(message.data()), message.size(), sk);
    return b64(sig, 64);
}
