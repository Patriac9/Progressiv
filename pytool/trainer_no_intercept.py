...
horizon = 8  # Position lasting time (seconds)
TAKER_FEE = 0  # flip / horizon / eod 吃单
MAKER_FEE = 0  # 建仓 / TP 挂单
QTY = 2  # 1 张标的
TP_OFFSET = 0.13  # 止盈/止损距离：开仓价 ± offset
...
intercept = 0.0
weights = dict(zip(active_features, coef))
...
result_path = result_dir / (datetime.now().strftime("%Y%m%d%H%M%S") + "_no_intercept")
...
try:
    ...
    print(f"T intercept={intercept:.6g}  (forced zero)")
...
t_path = models_dir / "T_Param_no_intercept"
alpha_path = models_dir / "alpha_Param_no_intercept"
